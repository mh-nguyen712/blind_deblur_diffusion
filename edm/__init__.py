...
def load_local_model(model_name: str = "edm-afhqv2-64x64-uncond-ve.pkl") -> nn.Module:
    r"""
    model_name (str): one of the following
        edm-afhqv2-64x64-uncond-ve.pkl
        edm-afhqv2-64x64-uncond-ve.pkl
    ...
    """
    sys.path.append(current_path)
    model_path = os.path.join(current_path, f'edm_ckpt/{model_name}')
    with open_url(model_path) as f:
        net = pickle.load(f)["ema"]
        print(f"Sucessfully loaded the model {model_name}")
        print(
            "Number of parameters: ",
