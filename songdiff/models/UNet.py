...
# ====================
# CNN-based Unet
# Clone from DPIR: https://github.com/cszn/DPIR
# Authors: Kai Zhang et al.
# Paper: Deep Plug-and-Play Image Restoration
...
class UNetResSuperResolution(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=[64, 128, 256, 512], nb=2, scale = 2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', bias=False):
        super().__init__()
        self.unet = UNetRes(in_nc, out_nc, nc, nb, act_mode, downsample_mode, upsample_mode, bias)
        ...
        self.scale = scale
        if scale > 1:
            self.upscaler = B.sequential(upsample_block(nc[0], in_nc, bias=bias, mode=str(scale)), *[B.ResBlock(in_nc, in_nc, bias=bias, mode='C'+act_mode+'C') for _ in range(nb)])
        else: 
            self.upscaler = B.conv(nc[0], out_nc, bias=bias, mode='C')

        self.unet.m_tail = nn.Identity()
    def forward(self, inputs):
        x = self.unet(inputs)
        return torch.sigmoid(self.upscaler(x))

 
class UNetResLite(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[8, 16, 32], nb=1, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(UNetResLite, self).__init__()

        self.m_head = B.conv(in_nc, nc[0], bias=True, mode='C')
        ...
        self.m_down1 = B.sequential(*[B.ResBlock(nc[0], nc[0], bias=True, mode='C'+act_mode+'C') for _ in range(nb)], downsample_block(nc[0], nc[1], bias=False, mode='2'))
        self.m_down2 = B.sequential(*[B.ResBlock(nc[1], nc[1], bias=True, mode='C'+act_mode+'C') for _ in range(nb)], downsample_block(nc[1], nc[2], bias=False, mode='2'))

        self.m_body  = B.sequential(*[B.ResBlock(nc[2], nc[2], bias=True, mode='C'+act_mode+'C') for _ in range(nb)])
